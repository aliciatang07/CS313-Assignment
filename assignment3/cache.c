...
static void get_shift_and_mask(int value, unsigned int *shift, uint64_t *mask,
    ...
    *shift = init_shift;
    ...
    //printf("mask is %2x, shift is %2x" shift, mask);
...
static int cache_line_check_validity_and_tag(cache_line_t *cache_line, intptr_t tag) {
    ...
    return (tag == cache_line->tag && cache_line->is_valid)?1:0;
}
...
static long cache_line_retrieve_data(cache_line_t *cache_line, size_t offset) {
    ...
    long result=0;
    for(i=0;i<4;i++){
        result+=cache_line->data[offset+i]<<(8*i);
    }
    return result;
}
...
static cache_line_t *find_available_cache_line(cache_t *cache, cache_set_t *cache_set) {
    ...
    //no available line, use replacement policy
    if((cache->policies&CACHE_REPLACEMENTPOLICY_MASK) == CACHE_REPLACEMENTPOLICY_LRU)
    {
        ...
        int random_line = rand()%cache->associativity;
        return cache_line_make_mru(cache_set,random_line);
    }
...
long cache_read(cache_t *cache, long *address) {
    ...
    int set_index = ((intptr_t)address>>cache->cache_index_shift)&cache->cache_index_mask;
    int offset =(intptr_t)address&cache->block_offset_mask;
    
    cache_set_t *cache_set= &cache->sets[set_index];
    cache_line_t *matched_cache_line = cache_set_find_matching_line(cache,cache_set,tag);
    
    if(matched_cache_line!=NULL){
        if(!cache_line_check_validity_and_tag(matched_cache_line,tag)){
            cache->miss_count++;
            matched_cache_line = cache_set_add(cache, cache_set, (intptr_t)address,tag);
        }
    }
    else{
        cache->miss_count++;
    ...
    return cache_line_retrieve_data(matched_cache_line,offset);
}
